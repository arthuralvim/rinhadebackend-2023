...
def get_nascimento_valido():
    return (
        str(random.randint(1940, 2020))
        + "-"
        + str(random.randint(1, 12 + 1)).rjust(2, "0")
        + "-"
        + str(random.randint(1, 28 + 1)).rjust(2, "0")
    )
...
def get_payload():
    ...
    get_nascimento = (
        get_nascimento_valido if gera_nascimento_valido else get_nascimento_invalido
    )
...
def gera_payloads(numero_registros):
    ...
    for _ in range(numero_registros):
        sys.stdout.write(f"{get_payload()}\n")


gera_payloads(300_000)
